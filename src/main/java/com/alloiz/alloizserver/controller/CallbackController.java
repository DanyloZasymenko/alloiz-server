...
public class CallbackController {
  ...
  private ResponseEntity<CallbackFullDto> save(@RequestBody CallbackFullDto callback) {
    return ResponseEntity
        .ok(map(callbackService.save(map(callback, Callback.class)), CallbackFullDto.class));
  }

  @PostMapping("/update")
  private ResponseEntity<CallbackFullDto> update(@RequestBody CallbackFullDto callback) {
    return ResponseEntity
        .ok(map(callbackService.update(map(callback, Callback.class)), CallbackFullDto.class));
  }
